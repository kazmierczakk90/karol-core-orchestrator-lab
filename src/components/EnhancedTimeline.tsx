...
import { Clock, Bot, Zap, CheckCircle, AlertCircle, Calendar, Play, Pause, Settings, Brain, MessageSquare, Command } from 'lucide-react';
...
const EnhancedTimeline = ({
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      ...
      case 'system':
        return 'bg-cyan-500/30 border-cyan-500/50';
      default:
  ...
  const renderTimelineItem = (event: TimelineEvent, index: number) => <div key={event.id} className="relative">
      {/* Timeline line */}
      {index < (event.type === 'live' ? liveEvents.length - 1 : plannedEvents.length - 1) && <div className="absolute left-6 top-16 w-0.5 h-16 bg-slate-600" />}
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-2">
                ...
                <span className="text-xs text-slate-400">
                  {event.type === 'live' ? `${Math.round((Date.now() - event.timestamp.getTime()) / 60000)}m ago` : `in ${Math.round((event.timestamp.getTime() - Date.now()) / 60000)}m`}
                </span>
              </div>
              
              {event.agentId && <Badge variant="outline" className="text-xs text-cyan-400 border-cyan-500/50">
                  {event.agentId}
                </Badge>}
            </div>
          </div>
  ...
  return <div className={`grid grid-cols-2 gap-6 ${className}`}>
      {/* Live Events Column */}
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button onClick={() => setIsAutoRefresh(!isAutoRefresh)} className={`p-1 rounded ${isAutoRefresh ? 'text-green-400' : 'text-slate-400'}`}>
        ...
        <CardContent className="p-0">
          <ScrollArea className="h-[500px]"> {/* 30% wyższe kontenery */}
            <div className="p-4 py-[28px]">
              {liveEvents.length > 0 ? liveEvents.map((event, index) => renderTimelineItem(event, index)) : <div className="text-center py-8">
      ...
      <Card className="bg-slate-800/50 border-slate-700/50">
        ...
        <CardContent className="p-0">
          <ScrollArea className="h-[500px]"> {/* 30% wyższe kontenery */}
            <div className="p-4">
              {plannedEvents.length > 0 ? plannedEvents.map((event, index) => renderTimelineItem(event, index)) : <div className="text-center py-8">
