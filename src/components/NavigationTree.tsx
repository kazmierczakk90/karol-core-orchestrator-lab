...
import { ChevronDown, ChevronRight, Folder, File, Bot, Code, Image, Settings, Search, Filter, RefreshCw } from 'lucide-react';
interface TreeNode {
  id: string;
  name: string;
...
const NavigationTree = ({
    ...
    const newTreeData: TreeNode[] = [{
      ...
      children: [{
        ...
        children: [{
          ...
          metadata: {
            status: 'active',
            description: 'Auto-improvement analysis'
          }
        }, {
          id: 'guardian-agent',
          name: '@guardian-core',
      ...
      children: [{
        ...
        children: [{
          ...
          metadata: {
            status: 'active',
            description: 'Mini AI management'
          }
        }, {
          id: 'agent-commander',
        ...
        children: [{
          ...
          metadata: {
            status: 'active',
            description: 'Web browsing functionality'
          }
        }, {
          id: 'workflow-builder',
  ...
  const getNodeIcon = (node: TreeNode) => {
    switch (node.type) {
      ...
      case 'file':
        return node.metadata?.fileType === 'tsx' ? <Code className="h-4 w-4 text-blue-400" /> : <File className="h-4 w-4 text-slate-400" />;
      default:
  ...
  const renderTreeNode = (node: TreeNode, level: number = 0) => {
    ...
    return <div key={node.id} className="select-none">
        <div className={`flex items-center space-x-2 py-2 px-3 rounded-md cursor-pointer transition-colors hover:bg-slate-700/50 ${isSelected ? 'bg-cyan-500/20' : ''}`} style={{
        paddingLeft: `${level * 20 + 12}px`
      }} onClick={() => handleNodeClick(node)}>
          <Checkbox checked={isSelected} onCheckedChange={() => toggleChecked(node.id)} onClick={e => e.stopPropagation()} className="h-4 w-4" />
          {getNodeIcon(node)}
          <span className="text-white text-sm flex-1">{node.name}</span>
          {node.metadata?.status && <Badge className={`text-xs ${getStatusColor(node.metadata.status)}`}>
              {node.metadata.status}
            </Badge>}
        </div>

        {node.children && isExpanded && <div>
            {filteredChildren?.map(child => renderTreeNode(child, level + 1))}
          </div>}
      </div>;
  };
  const handleGroupOperation = (operation: string) => {
    const selectedNodesArray = treeData.flatMap(node => getAllNodes(node)).filter(node => selectedNodes.has(node.id));
  ...
  return <Card className={`bg-slate-800/50 border-slate-700/50 ${className}`}>
      <CardHeader className="pb-3 py-0 my-0">
        ...
        <div className="space-y-2">
          <div className="relative">
            ...
            <input type="text" placeholder="Search nodes..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 bg-slate-700/50 border border-slate-600 rounded-md text-white text-sm" />
          </div>
          
          <div className="flex items-center space-x-2">
            <Filter className="h-4 w-4 text-slate-400" />
            <select value={filterType} onChange={e => setFilterType(e.target.value)} className="bg-slate-700/50 border border-slate-600 rounded-md px-3 py-1 text-white text-sm">
        ...
        {/* Group Operations */}
        {selectedNodes.size > 0 && <div className="flex items-center space-x-2 pt-2 border-t border-slate-600">
      ...
      <CardContent className="p-0">
        <ScrollArea className="h-[600px]">
          <div className="p-3 px-[46px] py-[5px]">
